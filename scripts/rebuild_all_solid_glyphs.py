...
# -*- coding: utf-8 -*-
"""
scripts/rebuild_all_solid_glyphs.py
===================================

Generates 100% solid, filled, non-hollow glyphs for all 113 Turkish characters,
digits, symbols, and currency marks with exact baseline alignment.
"""
...
GRID_SIZE = 32
UPM = 1000
CELL_SIZE = UPM // GRID_SIZE  # 31
BASELINE_ROW = 25

# Load Windows Segoe UI Bold or Arial Bold for solid template rendering
font_candidates = [
...
if not base_font_path:
    raise RuntimeError("No system bold font found for template generation!")

base_font = ImageFont.truetype(base_font_path, 21)

# Measure baseline offset for uppercase H
test_img = Image.new("1", (GRID_SIZE, GRID_SIZE), 0)
test_draw = ImageDraw.Draw(test_img)
bbox_H = test_draw.textbbox((0, 0), "H", font=base_font)
h_bottom = bbox_H[3]
oy_baseline = BASELINE_ROW - h_bottom
...
def grid_to_solid_contours(grid, cell_w=31, cell_h=31):
    """Converts every solid '#' grid block into an independent CCW rectangle contour."""
    contours = []
    rows = len(grid)
...
def main():
    # Remove all existing json files
    ...
    for name, char, u, cat, adv in CHARS_DATA:
        ...
        if u and u > 32:
            ...
            draw.text((ox, oy_baseline), char, font=base_font, fill=1)
        ...
        contours = grid_to_solid_contours(grid_strings, CELL_SIZE, CELL_SIZE)
    ...
    notdef_data = {
        ...
        "gridCols": GRID_SIZE,
        "gridRows": GRID_SIZE,
        "contours": grid_to_solid_contours(notdef_grid, CELL_SIZE, CELL_SIZE),
        "strokes": [],
        "grid": notdef_grid,
    }
